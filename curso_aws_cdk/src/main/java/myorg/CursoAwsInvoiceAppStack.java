...
public class CursoAwsInvoiceAppStack extends Stack {
    private final Bucket bucket;
    private final Queue s3InvoiceQueue;
    public CursoAwsInvoiceAppStack(final Construct scope, final String id) {
        this(scope, id, null);
    }

    public CursoAwsInvoiceAppStack(final Construct scope, final String id, final StackProps props) {
        super(scope, id, props);
        SnsTopic s3InvoiceTopic = SnsTopic.Builder.create(Topic.Builder.create(this, "s3InvoiceTopic")
                .topicName("s3-invoice-events")
                .build())
                .build();
        bucket = Bucket.Builder.create(this, "S301").bucketName("pcs01-invoice").removalPolicy(RemovalPolicy.DESTROY).build();
        ...
        s3InvoiceQueue = Queue.Builder.create(this, "S2InvoiceQueue")
